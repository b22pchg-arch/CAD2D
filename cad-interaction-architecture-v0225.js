...
(()=>{
  ...
  if(previousMirrorSelected){const wrapped=function(a,b){if(!(selected||[]).length){try{status('Hãy chọn đối tượng trước khi dùng Đối xứng.');setTool('select')}catch{}return}if(typeof dist==='function'&&dist(a,b)<1e-9){try{status('Trục đối xứng không hợp lệ.')}catch{}return}const ref=selectionCenter(),referenceBounds=window.DwgSketchMacroEngineV02210?.captureReferenceBounds?.(),tx=captureSelectionDelta('MIRROR');try{selected.forEach(s=>mirrorItem(s.item,a,b));recalcBounds();finalizeSelectionDelta(tx,`Đã MIRROR ${selected.length} đối tượng bằng transaction delta.`);window.DwgSketchMacroEngineV02210?.recordRelativeTransform?.('RMIRROR',{axisAOffsetX:a.x-ref.x,axisAOffsetY:a.y-ref.y,axisBOffsetX:b.x-ref.x,axisBOffsetY:b.y-ref.y,referenceBounds});try{syncElectricalAutomationFromSymbols()}catch{}setTool('select');updateSelectionPanel();draw()}catch(err){cancelTransaction();throw err}};try{mirrorSelected=wrapped}catch{window.mirrorSelected=wrapped}}
