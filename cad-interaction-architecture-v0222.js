'use strict';
(()=>{
  const API_VERSION='0.22.2',DELTA_KIND='selection-delta-v0222';
  const state={
  ...
  const clearBtn=document.getElementById('clearSelectionBtn');if(clearBtn)clearBtn.onclick=()=>{clearSelection();try{updateSelectionPanel();updateHistoryButtons();draw();status('Đã bỏ chọn toàn bộ đối tượng qua Selection Service V0.22.2.')}catch{}};
  ...
  window.DwgSketchCadArchitectureV0222={version:API_VERSION,registerCommand,snapshot,health,report,reportTransactions,beginTransaction,commitTransaction,cancelTransaction,
