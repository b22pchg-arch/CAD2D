...
(() => {
  ...
  function searchScore(t,query){
    ...
    const name=normalizeSearch(t.name),keywords=normalizeSearch(t.keywords),source=normalizeSearch(t.sourceFile),all=`${name} ${keywords} ${source}`,tokens=query.split(' ').filter(Boolean);
    if(tokens.some(token=>!all.includes(token)))return-1;
    let score=0;
    for(const token of tokens)score+=name===token?120:name.startsWith(token)?70:name.includes(token)?45:keywords.includes(token)?25:12;
  ...
  function installUi(){
    ...
    box.innerHTML=`<h3>Thư viện mẫu sơ đồ A4 có thể chỉnh sửa — V0.17.7</h3>
      <div class="muted">Lưu vùng chọn hoặc toàn bản vẽ, chuẩn hóa A4, tìm nhanh không phân biệt dấu và chèn bằng preview nhẹ.</div>
  ...
  const api={version:API_VERSION,schema:SCHEMA,getCatalog:()=>normalizeCatalog(deep(catalog)),saveSelection:()=>saveTemplate('selection'),saveDrawing:()=>saveTemplate('drawing'),insert:startInsert,cancel:cancelPending,refresh:renderList,toggleFrameSelection:()=>command('A4FRAMESEL'),selectInstance:selectInsertedInstance};
  window.DwgSketchA4DiagramTemplateCoreV0217=api;
