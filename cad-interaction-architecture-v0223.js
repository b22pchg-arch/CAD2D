'use strict';
(()=>{
  const API_VERSION='0.22.3',DELTA_KIND='selection-delta-v0223',STRUCTURAL_KIND='structural-delta-v0223';
  const state={
  ...
  function undoArchitectureDelta(){
    ...
    if(e?.kind!==DELTA_KIND&&e?.kind!==STRUCTURAL_KIND)return false;undoStack.pop();
    const ok=e.kind===STRUCTURAL_KIND?applyStructural(e,false):applyDelta(e,false);if(!ok){undoStack.push(e);return true}
    if(!Array.isArray(redoStack))redoStack=[];redoStack.push(e);if(typeof trimAdaptiveHistory==='function')trimAdaptiveHistory(redoStack);state.historyUndo++;try{status(`Đã Undo ${e.name} bằng ${e.kind===STRUCTURAL_KIND?'structural':'transaction'} delta.`)}catch{}return true;
  }
  function redoArchitectureDelta(){
    ...
    if(e?.kind!==DELTA_KIND&&e?.kind!==STRUCTURAL_KIND)return false;redoStack.pop();
    const ok=e.kind===STRUCTURAL_KIND?applyStructural(e,true):applyDelta(e,true);if(!ok){redoStack.push(e);return true}
    if(!Array.isArray(undoStack))undoStack=[];undoStack.push(e);if(typeof trimAdaptiveHistory==='function')trimAdaptiveHistory(undoStack);state.historyRedo++;try{status(`Đã Redo ${e.name} bằng ${e.kind===STRUCTURAL_KIND?'structural':'transaction'} delta.`)}catch{}return true;
  }
  ...
  const clearBtn=document.getElementById('clearSelectionBtn');if(clearBtn)clearBtn.onclick=()=>{clearSelection();try{updateSelectionPanel();updateHistoryButtons();draw();status('Đã bỏ chọn toàn bộ đối tượng qua Selection Service V0.22.3.')}catch{}};
  ...
  window.DwgSketchCadArchitectureV0223={version:API_VERSION,registerCommand,snapshot,health,report,reportTransactions,beginTransaction,commitTransaction,cancelTransaction,
    selection:{clear:clearSelection,replace:replaceSelection,single:selectSingle,toggle:toggleSelection,sync:syncSelection},
    transaction:{captureSelectionDelta,finalizeSelectionDelta,noteDelta,beginStructuralAdd,commitStructuralAdd,cancelStructuralAdd,beginStructuralRemove,commitStructuralRemove,cancelStructural,applyStructural},noteSelection:syncSelection,noteGeometry(){state.geometryInvalidations++;state.entityRevision++},noteRender(){state.renderRequests++}};
