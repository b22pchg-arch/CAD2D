'use strict';
(()=>{
  const API_VERSION='0.22.4',DELTA_KIND='selection-delta-v0224',STRUCTURAL_KIND='structural-delta-v0224',CURVE_KIND='curve-edit-delta-v0224';
  const state={
  ...
  const clearBtn=document.getElementById('clearSelectionBtn');if(clearBtn)clearBtn.onclick=()=>{clearSelection();try{updateSelectionPanel();updateHistoryButtons();draw();status('Đã bỏ chọn toàn bộ đối tượng qua Selection Service V0.22.4.')}catch{}};
  ...
  window.DwgSketchCadArchitectureV0224={version:API_VERSION,registerCommand,snapshot,health,report,reportTransactions,beginTransaction,commitTransaction,cancelTransaction,
